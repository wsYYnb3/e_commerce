...
export const schema = yup.object().shape({
    billingDetails: yup.object().shape({
      ...
      phone: yup.object().shape({
        countryCode: yup.string().matches(/^\+?\d{1,3}$/, 'International country code not valid ').required('Area code is required'),
      ...
      address: yup.object().shape({
        ...
        number: yup.string().matches(/^[0-9]+$/, 'Street number is not valid').required('Street number is required'),
        apt: yup.string(),
